...
# 1. API CONFIGURATION
# ---------------------------------------------------------
# ⚠️ 여기에 새로 발급받은 API 키를 넣으세요
os.environ["GOOGLE_API_KEY"] = "AIzaSyAxLuq8wtcbr_cLjXcOZ3scIOgNZV_lTs4"
...
# 2. UI DESIGN
# ---------------------------------------------------------
st.set_page_config(page_title="Intelligent AI Chef", page_icon="👨‍🍳")
st.title("👨‍🍳 Intelligent AI Chef")
...
# User Inputs
st.sidebar.header("📍 Kitchen Environment")
ingredients = st.sidebar.text_area("🛒 Available Ingredients", "Eggs, Onion, Kimchi, Rice")
cooking_time = st.sidebar.select_slider("⏱ Max Time (min)", options=[5, 10, 15, 20, 30, 45, 60], value=15)
tools = st.sidebar.multiselect("🍳 Available Tools", 
...
# 4. EXECUTION
# ---------------------------------------------------------
if st.button("🚀 Generate Logical Recipe"):
    if not ingredients:
        ...
        with st.spinner("Chef is reasoning through your situation..."):
            try:
                ...
                if "[YouTube Query:" in content:
                    ...
                    st.divider()
                    st.info(f"📺 **Verified Video Search:** [{query_raw}]({yt_url})")
